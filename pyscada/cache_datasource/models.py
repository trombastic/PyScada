...
class DjangoCache(models.Model):
    ...
    def __str__(self):
        return f"Django Cache ({self.data_lifetime}.)"
    ...
    def _update_variable_data(self, variable_id: int, new_data=None, date_saved=None):
        ...
        for key in list(data.keys()):
            if key < (self.now_ms() - self._data_lifetime_ms):
                logger.error(
                    f"{key} smaler then {(self.now_ms() - self._data_lifetime_ms)}"
                )
                del data[key]
    ...
    def query_datapoints(
        ...
        **kwargs,
        ...
        for variable_id in variable_ids:
            ...
            for timestamp_ms, item in data.items():
                ...
                if timestamp < time_min or (
                    ...
                    if last_value is None:
                        last_value = [timestamp, value, date_saved]
                    elif last_value[0] < timestamp:
                        last_value = [timestamp, value, date_saved]
                    continue
            ...
            if query_first_value and last_value is not None:
                if variable_id not in output:
                    output[variable_id] = []
                # prepend last value
                output[variable_id] = [last_value[0:2]] + output[variable_id]
                output["timestamp"] = max(output["timestamp"], last_value[0])
                output["date_saved_max"] = max(output["date_saved_max"], last_value[2])
