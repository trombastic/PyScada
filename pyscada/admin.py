...
class DeviceListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        in the right sidebar.
        """
        devices = set([d.device for d in model_admin.model.objects.filter(calculatedvariableselector__isnull=False)])
...
class VariableListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        in the right sidebar.
        """
        variables = set([v for v in model_admin.model.objects.filter(calculatedvariableselector__isnull=False)])
    ...
    def queryset(self, request, queryset):
        ...
        """
        if self.value():
            return queryset.filter(calculatedvariable__variable_calculated_fields__main_variable_id=self.value())
...
class ProtocolListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        in the right sidebar.
        """
        protocols = set([p.device.protocol for p in model_admin.model.objects.filter(calculatedvariableselector__isnull=False)])
...
class VariableAdminFrom(forms.ModelForm):
        ...
        if isinstance(self.instance, Variable):
            ...
            if "chart_line_color" in self.fields:
                ...
                def create_option_color(self, name, value, label, selected, index, subindex=None, attrs=None):
                    ...
                    return self._create_option(name,
                                               value,
                                               label,
                                               selected,
                                               index,
                                               subindex,
                                               attrs={f"style": "background: {label}; color: #{font_color}"})
    ...
    def has_changed(self):
        ...
        if self.data.get("device", None) != "" \
           and self.data.get("device", None) is not None:
            ...
            if hasattr(self.instance, "protocol_id") \
               and d is not None \
    ...
    def clean(self):
        ...
        if self.has_changed() \
           ...
           and "device" in self.changed_data:
            ...
            for v in related_variables:
                if hasattr(self.instance, v.name) \
                   ...
                   and getattr(self.instance, v.name).protocol_id != self.cleaned_data["device"].protocol.id:
...
class DeviceForm(forms.ModelForm):
    def has_changed(self):
        ...
        if self.data.get("protocol", None) is not None \
           and hasattr(self.instance, "protocol_id") \
           and self.data.get("protocol", None) == str(self.instance.protocol_id):
                return True
        else:
            if hasattr(self.instance, "protocol_id") \
               ...
               and self.instance.parent_device().protocol.id == self.instance.protocol_id:
                return True
        return super().has_changed()
...
class VariableAdmin(admin.ModelAdmin):
    ...
    def last_value(self, instance):
        element = RecordedData.objects.last_element(variable_id=instance.pk)
        if element:
            return f"{datetime.datetime.fromtimestamp(element.time_value()).strftime('%Y-%m-%d %H:%M:%S')} : {element.value().__str__()} {instance.unit.unit}"
        else:
            return f" - : NaN {instance.unit.unit}"
    ...
    def get_queryset(self, request):
        ...
        if "environ" in request.__dict__:
            for i in request.__dict__["environ"]:
                if isinstance(request.__dict__["environ"][i], str) \
...
class CoreVariableAdmin(VariableAdmin):
    list_display = ("id", "name", "description", "unit", "scaling", "device",
                    "value_class", "active", "writeable","dictionary",)
...
class CalculatedVariableAdmin(VariableAdmin):
    list_display = ("name", "last_check", "state", "last_value",
                    "chart_line_color", "short_name",)

    list_editable = ("chart_line_color", "short_name",)
    list_filter = (ProtocolListFilter, DeviceListFilter, VariableListFilter,
...
class ScalingAdmin(admin.ModelAdmin):
    list_display = ("id", "description", "input_low", "input_high",
                    "output_low", "output_high", "limit_input",)

    list_editable = ("input_low", "input_high", "output_low", "output_high",
                    "limit_input",)
...
class DeviceWriteTaskAdmin(admin.ModelAdmin):
    list_display = ("id", "name", "value", "user_name", "start_time", "done",
                    "failed",)
...
class BackgroundProcessAdmin(admin.ModelAdmin):
    list_display = ("id", "pid", "label", "message", "last_update",
                    "running_since", "enabled", "done", "failed")

    list_filter = (BackgroundProcessFilter, "enabled", "done", "failed")
    list_display_links = ("id", "label", "message")
    readonly_fields = ("message", "last_update", "running_since", "done",
                       "failed")
...
class RecordedEventAdmin(admin.ModelAdmin):
    list_display = ("id", "event", "complex_event", "time_begin", "time_end",
                    "active",)
...
class MailAdmin(admin.ModelAdmin):
    list_display = ("id", "subject", "message", "html_message", "last_update",
                    "done", "send_fail_count",)
...
class ComplexEventInputAdminInline(admin.StackedInline):
    ...
    fieldsets = (
        (None,
            {
                "fields": (
                    ("fixed_limit_low", "variable_limit_low", "limit_low_type", "hysteresis_low",),
...
class ComplexEventLevelAdmin(admin.ModelAdmin):
    list_display = ("id", "level", "send_mail", "complex_event", "order",
                    "active",)
...
class ComplexEventInputAdmin(admin.ModelAdmin):
    list_display = ("id", "fixed_limit_low", "variable_limit_low",
                    "limit_low_type", "hysteresis_low", "variable",
                    "fixed_limit_high", "variable_limit_high",
                    "limit_high_type", "hysteresis_high",)
...
class VariablePropertyAdmin(admin.ModelAdmin):
    list_display = ("id", "variable", "name", "property_class", "value",
