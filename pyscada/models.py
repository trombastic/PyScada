...
class Color(models.Model):
    ...
    def color_rect_html(self):
        return f'<div style="width:4px;height:0;border:5px solid #{self.R:02x}{self.G:02x}{self.B:02x};overflow:hidden"></div>'
...
class Device(models.Model):
    ...
    def __str__(self):
        ...
        if self.protocol is not None:
            ...
            return f"generic-{self.short_name}"
    ...
    def get_device_instance(self):
        try:
            ...
            logger.error(f"{self.short_name}({getpid()}), unhandled exception\n{traceback.format_exc()}")
...
class Scaling(models.Model):
    ...
    def __str__(self):
        if self.description:
            ...
            return f"{str(self.id)}_[{str(self.input_low)}:{str(self.input_high)}] -> [{str(self.output_low)}:{str(self.output_low)}]"
...
class Dictionary(models.Model):
    ...
    def __str__(self):
        return f"{text_type(str(self.id))}: {self.name}"
        ...
        for item in self.dictionaryitem_set.all():
            if float(item.value) == float(value):
                if label_found is None:
                    ...
                    logger.info(f"Dictionary {str(self)} has various items with value = {value}")
                    return None
    ...
    def append(self, label, value, silent=False):
        try:
            ...
            if not silent:
                logger.warning(f"Item ({label}:{value}) for dictionary {self} already exist")
...
class DictionaryItem(models.Model):
    ...
    def __str__(self):
        return f"{text_type(str(self.id))}: {self.label}"
...
class VariableProperty(models.Model):
    ...
    def __str__(self):
        return f"{self.get_property_class_display()}: {self.name}"
    ...
    def web_key(self):
        return f"{self.variable.pk}-{self.name.upper().replace(':', '-')}"
    ...
    def convert_string_value(self, value):
        if self.dictionary is None:
            d = Dictionary(name=f"{str(self.name)}_auto_created")
            d.save()
            self.dictionary = d
        ...
        if not len(self.dictionary.dictionaryitem_set.filter(label=str(value))):
            ...
            logger.warning(f"{len(self.dictionary.dictionaryitem_set.filter(label=str(value)))} duplicate values found of {value} in dictionary {self.dictionary}")
...
class Variable(models.Model):
    ...
    def update_value(self, value=None, timestamp=None):
        ...
        try:
            ...
            if type(value) == str:
                ...
                logger.info(f"Value read for {self} format not supported : {type(value)}")
                value = None
        except TypeError:
            pass
    ...
    def _send_cov_notification(self, timestamp, value):
        ...
        try:
            ...
            logger.error(f"{self.name}, unhandled exception in COV Receiver application\n{traceback.format_exc()}")
    ...
    def convert_string_value(self, value):
        try:
            ...
            if not len(self.dictionary.dictionaryitem_set.filter(label=str(value))):
                ...
                logger.warning("{len(self.dictionary.dictionaryitem_set.filter(label=str(value)))} duplicate values found of {value} in dictionary {self.dictionary}")
...
class PeriodicField(models.Model):
    ...
    #fixme this overides the buildin type methode!
...
class CalculatedVariable(models.Model):
    ...
    def check_period(self, d1, d2, force_write=False, add_partial_info=False):
        self.state = f"Checking [{d1} to {d2}]"
        ...
        if self.period_diff_quantity(d1, d2) is None:
            self.state = f"[{d1} to {d2}] < {str(self.period.period_factor) + self.period.period_choices[self.period.period][1]}"
            self.state = self.state[0:100]
            self.save(update_fields=["state"])
            return None
        ...
        if d is None:
            self.state = f"No time range found [{d1} to {d2}] {self.period}"
            self.state = self.state[0:100]
            self.save(update_fields=["state"])
            return None
        [d1, d2] = d
        ...
        if self.period_diff_quantity(d1, d2) is None:
            logger.debug(f"No period in new date interval : {self.period} ({d1} {d2})")
        ...
        while d2 >= d1 + td and d1 + td <= now():
            ...
            if not force_write and len(v_stored) and len(v_stored[self.store_variable.id][0]):
                logger.debug(f"Value already exist in RecordedData for {d1} - {d1 + td}")
            ...
            if type_str == "min":
                ...
                if p == "" or p is None or p == "None":
                    ...
                    try:
                        ...
                        logger.warning(f"Period field {self.period} property after < is not a float : {self.period.property}")
                        res = None
                else:
                    try:
                        p = float(p)
                        ...
                        logger.warning(f"Period field {self.period} property is not a float : {self.period.property}")
                        res = None
            elif type_str == "max":
                p = str(self.period.property)
                if p == "" or p is None or p == "None":
...
class BackgroundProcess(models.Model):
    ...
    def get_process_instance(self):
        ...
        try:
            ...
            logger.error(f"{self.label}({getpid()}), unhandled exception\n{traceback.format_exc()}")
            return None
    ...
    def restart(self):
        ...
        if self.pid != 0 and self.pid is not None:
            ...
            try:
                ...
                logger.debug(f"{self.pk}: send SIGUSR1 to {self.pid}")
                return True
            except OSError as e:
                return False
    ...
    def _stop(self, signum=signal.SIGTERM):
        ...
        """
        if self.pid != 0 and self.pid is not None:
            logger.debug(f"send {signum} to daemon")
            try:
                kill(self.pid, signum)
            except OSError as e:
                if e.errno == errno.ESRCH:
                    try:
                        logger.debug(f"{self}: process id {self.pid} is terminated")
                        return True
                    except:
                        return False
            try:
...
class ComplexEventInput(models.Model):
    ...
    variable_limit_high = models.ForeignKey(Variable, blank=True, null=True,
